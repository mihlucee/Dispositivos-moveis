...
const UpcomingMoviesScreen = ({ navigation }) => {
  ...
  const loadUpcomingMovies = useCallback(async () => {
    ...
    try {
      ...
      setError('Não foi possível carregar próximos lançamentos. Verifique sua conexão ou chave da API.');
      console.error('Erro ao carregar próximos lançamentos na tela:', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const renderItem = ({ item }) => (
    <Card
      ...
      onPress={() => navigation.navigate('DetalhesProximos', { item })}
    >
      {item.poster_path ? (
        <Card.Cover
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <Appbar.Header style={{ backgroundColor: theme.colors.primary, elevation: 2 }}> 
        <Appbar.Content title="Próximos Lançamentos" titleStyle={{ fontSize: 24, color: theme.colors.buttonText }} /> 
      </Appbar.Header>

      {loading ? (
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color={theme.colors.primary} />
          <Text variant="bodyLarge" style={{ color: theme.colors.text }}>Carregando próximos lançamentos...</Text>
        </View>
      ) : error ? (
        <View style={styles.errorContainer}>
          <Text variant="bodyLarge" style={[styles.errorText, { color: theme.colors.error }]}>{error}</Text>
          <Button onPress={loadUpcomingMovies} mode="contained" style={{ backgroundColor: theme.colors.primary }}>
            Tentar Novamente
          </Button>
        </View>
      ) : movies.length === 0 ? (
        <View style={styles.emptyContainer}>
          <Text variant="headlineMedium" style={{ color: theme.colors.text }}>Nenhum próximo lançamento encontrado.</Text>
